...
//for xLocStor:
        ...
        addRules('.hp .sfsbc,.sfsbc{display: inline-block}.siteList:hover button{display: block}'
            ...
            +'div.gb_g[aria-label="promo"],.pdp-psy.og-pdp, .gb_Sc.gb_g .gb_ha, .gb_g.gb_ha:not(.xpdopen ){display: none}.xpdopen{display: block!important}.rhsvw{opacity:.16; transition:.4s}.rhsvw:hover{opacity:1}'
                    ...
                    if(buttSearch && top == self) for(var i in buttS) if(i !='site'|| S.sites){ //buttons under search input line
                        ...
                        var bI = buttS[i]
                            ...
                            ,at: {value: iD !=-1 && S.dwmyh[iD] !=1 ? S.dwmyh[iD] + bI.lett : i
                                ,innerHTML: '<div'+ (d16 ?' class=xButt2':'') +'><span class=txt onclick=this.parentNode.click();return!1 title="'
                                +(lang || i=='site'|| i=='.. : ..'
                            ...
                            ,cs: $x({position:'absolute', top:'33px',wordSpacing:'-1px'}, csLeft(++ii))
                            ,on: {click: (function(bI, i, iD){
                                //console.log('clic0:', i, iD);
                                return /PDF|DOC|site/.test(i)
                                    ? function(ev){
                                        var t = ev.target;
                                        console.log('cli-DocSite:',i,bI,ev, t.className, inputSearch.value, this.form, ', attr: ', t.getAttribute('site'), $LSettings, t.parentNode.getAttribute('site'), t.value);
                                        if(t.className =='defa')
                                        ...
                                        if(/xButt|txt/.test(t.className) && !(i=='site'&& !(/list/.test(t.parentNode.className)
                                            || /list/.test(t.parentNode.parentNode.className))) || /PDF|DOC/.test(t.value))
                                            buttSearch.click();
                                            ...
                                            var t = ev && ev.target, sbd = /,sbd:1/.test(lh),
                                                el2 = t && t.parentNode,
                                                date2 = el2.getAttribute('date');
                                            console.log('cli-Past:','/search?q='+ encodeURIComponent(inputSearch.value) +(date2 ||''), el2.className+','+t.className, el2.classList.contains('siteList'), t.classList.contains('list'));
											/*	+((t.getAttribute('date') || t.parentNode.getAttribute('date'))
											 + (t.value || t.parentNode.value).replace(/\D/g,'') || bI.url) );
											 var locationhref = '/search?q='+ encodeURIComponent(inputSearch.value)
											 +(el2.getAttribute('date')
											 + el2.value.replace(/\D/g,'') || bI.url)
											 +(/tbs=\w+(:|%3A)\w+(,|%2C)sbd(:|%3A)1/.test(lh) ?',sbd:1':'') //save "sort by date" option
											 +(/[&?]tbm=/.test(lh) ? '&'+/tbm=[^&]* /.exec(lh)[0]:''); //saving type of page*/
                                            if(el2.classList.contains('siteList') && !t.classList.contains('list')){ //clicked by top button
                                                var elTop = $q('div:not(.list) >.txt', el2) ||''
                                                    ,itrvNum = elTop && elTop.getAttribute('itrvNum') ||''
                                                    ,newSrch = lh.replace(/(&|\?)q=([^&]+)(&|$)/g,'$1q='+ encodeURIComponent(inputSearch.value) +'$3');
                                                if(layout1811 && itrvNum !=='') location.href = /qdr(:|%3A)([dwmyh])\d*/.test(lh)
                                                    ? newSrch.replace(/qdr(:|%3A)[dwmyh]\d*/, itrvNum)
                                                    : newSrch + (/\?/.test(newSrch) ?'&':'?') + itrvNum;
                                                if(itrvNum != null) S.dwmyh[iD] = +(itrvNum ||'').replace(/\D/g,'');
                                            }else if(t.textContent || el2.textContent)
                                                S.dwmyh[iD] = +(t.textContent || el2.textContent ||'').replace(/\D/g,'');
                                            console.log('==1', el2.value, itrvNum);
                                            $pd(ev);
                                            ev.stopPropagation();
                                            saveLocStor();
                                        }
                                ...
                                var sI = list[j]
                                    ...
                                    ,at:{value: sI
                                        ...
                                        ,innerHTML:'<span class=txt>'+ sI +'</span>'+ (sI != $LSettings &&!(!S.sites && i =='1H')
                                            ...
                                            +'</div>')}
                                    ,cs: {position: sI != $LSettings || design1612 || layout1811 ?'static':'absolute',display:'block', width: sI != $LSettings ?'auto': /en|es/.test(lang)||!lang ?'4em':'6.2em', height: sI != $LSettings ?'18px':'16px'
                                        ,margin:'2px 0 -1px -13px', padding:'0 2px 0 1px', textAlign:'left', fontWeight:'normal', opacity:1, whiteSpace:'nowrap'}
                                    ,on:{click: function(ev){
                                        var less = $q('#hoursLess')
                                            ,des16 = $q('#design1612')
                                            ,itrv = ev.target.getAttribute('date')
                                            ,num = ev.target.getAttribute('site').replace(/\D/g,'');
                                        console.log('c3',ev.target, itrv, num);
                                        ...
                                        if(des16 && /design1612/.test(ev.target.id)){
                                            ...
                                            saveLocStor();}
                                        var newSrch = lh.replace(/(&|\?)q=([^&]+)(&|$)/g,'$1q='+ encodeURIComponent(inputSearch.value) +'$3');
                                        if(layout1811 && num !=='') location.href = /qdr(:|%3A)([dwmyh])\d*/.test(lh)
                                            ? newSrch.replace(/qdr(:|%3A)[dwmyh]\d*/, itrv + num)
                                            : newSrch + (/\?/.test(newSrch) ?'&':'?') + itrv + num;
                                        $pd(ev);
                                    }}
                                    ,apT: siteList
                                });
