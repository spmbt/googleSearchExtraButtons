...
// ==/UserScript==
var xLocStI =1, xLocSto = [{origin:'https://spmbt.github.io', restHref:'/googleSearchExtraButtons/saveYourLocalStorage.html'},
	...
	{origin:'https://www.wikidata.org', restHref:'/wiki/Wikidata:Privacy_policy'}];
...
if(location.host == xLocSto[xLocStI].origin.replace(/[^/]*\/\//,'')){
	window.addEventListener('message', function(ev){
		...
		}},!1); console.log('[xLocSto-1]', location.host);
...
//for xLocStor:
	...
	addRules('.hp .sfsbc,.sfsbc{display: inline-block}.siteList:hover button{display: block}'
		...
		+'.sbibod >.siteList:hover,.lsbb >.siteList >div:not([class]):hover{background-color:#c2d4e0; color: #f7f7f7; opacity:.93}'
					...
					var bI = buttS[i]
						...
						,on: {click: (function(bI, i, iD){
							...
							return /Srch|PDF|DOC|site/.test(i)
								? function(ev){
									var t = ev.target;
									console.log('cli-DocSite:',i,bI,ev, t.className, inputSearch.value, this.form, ',' +
										' attr: ', t.getAttribute('site'), $LSettings, t.parentNode.getAttribute('site'), t.value);
									if(t.className =='defa')
									...
									inputSearch.value = (inputSearch.value||'').replace(/ site\:[\w.]*$/ig, function(a){return i=='Srch'? a :''})
											.replace(/( |\+|&as_)filetype(:|%3A)[^\&]*/g,'') +' '+ (/Srch|PDF|DOC/.test(i) ? bI.url
									...
									if(/xButt|txt/.test(t.className) && !(i=='site'&& !(/list/.test(t.parentNode.className)
										|| /list/.test(t.parentNode.parentNode.className))) || /Srch|PDF|DOC/.test(t.value))
										buttSearch.click();
										...
										console.log('cli-Past:','/search?q='+ encodeURIComponent(inputSearch.value) +(date2 ||''),
											tOvr.className+','+t.className, tOvr.classList.contains('siteList'), t.classList.contains('list'));
										/*	+((t.getAttribute('date') || t.parentNode.getAttribute('date'))
										 ...
										 +(tOvr.getAttribute('date')
										 + tOvr.value.replace(/\D/g,'') || bI.url)
										...
										if(tOvr.classList.contains('siteList') && !ta.classList.contains('list')){ //clicked by top button
											...
											S.dwmyh[iD] = +(t.textContent || tOv0.textContent ||'').replace(/\D/g,'');
							...
							var sI = list[j]
								...
								,at:{value: sI
									...
									,innerHTML:'<span class=txt>'+ sI +'</span>'+ (sI != $LSettings &&!(!S.sites && i =='1H')
										...
										+'</div>')}
								,cs: {position: sI != $LSettings || design1612 || layout1811 ?'static':'absolute',display:'block'
									,width: sI != $LSettings ?'auto': /en|es/.test(lang)||!lang ?'4em':'6.2em', height: sI != $LSettings ?'18px':'16px'
